...
class Kinematics
{
  ...
  private:
    ...
    vector<KDL::Jacobian> finger_tip_jac_;

    int perturbed_trials_;
    bool warned_perturbed_trials_;

    void createFingerSolvers_(InspireKdlConfig& kdl_config, const vector<double>& q_min, const vector<double>& q_max, unsigned int maxiter, double eps);
  ...
  public:
    ...
    **/
    ...
    void calcJac(const vector<double>& q);
    void getJac(vector<KDL::Jacobian>& Jacobian) {Jacobian=finger_tip_jac_;}
};
